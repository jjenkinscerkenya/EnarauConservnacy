...
STRICT_INPUT_CHECK <- TRUE  # Confirms all 60 input files are present
...
if (nrow(seasonal_available) > 0) {
  ...
  message("No seasonal rasters -- skipping seasonal coverage QA.")
}
...
if (nrow(period_available) > 0) {
  ...
  message("No period composite rasters -- skipping period coverage QA.")
}
